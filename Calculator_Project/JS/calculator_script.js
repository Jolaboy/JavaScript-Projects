...
const Calculator = {
    ...
    Display_Value: '0',
    //This will hold the first operand for any expressions, is set it to null for now.
    First_Operand: null,
    //This checks whether or not the second operand has been inputted by the user.
    Wait_Second_Operand: false,
    //This will hold the operator for any expressions, is set to null for now.
    operator: null,
};
...
function Input_Digit(digit) {
    ...
    //This checks if the wWait_Second_Operand is true and set Display_Value.
...
function Handle_Operator(Next_Operator) {
    ...
    //Calculator.First_Operand if it is doesn't already exists.
    ...
    //Checks if first operand is null and if it is, then sets it to the value of the input.
...
keys.addEventListener('click', (event) => {
    ...
    if (target.classList.contains('all-clear')) { // Add this block
        Calculator_Reset();
        Update_Display();
        return;
    }
